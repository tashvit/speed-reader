...
(function () {
  ...
  const createSpeedReadFunction = (n) => {
    const reader = (n) => {
      ...
      const selectionObject = '[starting]. [starting].. [starting]... [starting]....' + window.getSelection();
      ...
      if (n < selectionArray.length && switcher) {
        ...
        if (
          selectionArray[n].replace(/[?.,!:;*-]+$/, "") !== selectionArray[n]
        ) {
  ...
  const createButton = () => {
    ...
    button.style.top = "0";
	button.style.zIndex = "9999999999999999999999999";
    const overlay = document.querySelector(".overlay");
    if (!overlay) {
      createOverlay();
      createReaderWrapper();
  ...
  function onKeydown(evt) {
    ...
    const overlay = document.querySelector(".overlay");
    if (evt.key === "Escape" && !!overlay && overlay.style.display != "none") {
      const readerWrapper = document.querySelector(".reader-wrapper");
      overlay.style.display = "none";
      readerWrapper.style.display = "none";
      return;
